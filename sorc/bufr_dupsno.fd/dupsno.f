...
C$$$
     ...
     $ (c_missing,i_missing)
      ...
      DATA TSTR /'CLAT  CLON  MNTH DAYS HOUR MINU RPID SELV  TOSD'/
      DATA TSTH /'CLATH CLONH MNTH DAYS HOUR MINU RPID HSMSL TOSD'/
...
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      CALL W3TAGB('BUFR_DUPSNO',2022,0084,0054,'NP22')

      print *
      print * ,'---> Welcome to BUFR_DUPSNO - Version 05-22-2024'
      print *
...
2     CONTINUE
      IF(.NOT.SYNOP) THEN
         PRINT *
         PRINT *,'BUFR_DUPSNO: NO SYNOPTIC REPORTS TO CHECK'
         PRINT *
         CALL W3TAGE('BUFR_DUPSNO')
         STOP
      ELSE
         print 200, dexy,dday,dour
  200 FORMAT(/'BUFR_DUPSNO PARAMETERS:'/
...
C  --------------------------------------------------------
     ...
     .          'I5/)', I,MSGT
               PRINT'("#####BUFR_DUPSNO (UFBTAB) CANNOT PROCESS '//
...
cc     .             '; file #',I2,'} ELEV=',F15.2,'TOSD=',F15.5)
...
cc                  print *,'lon diff =',ABS(TAB_8(2,I)-TAB_8(2,J))
            ...
            if(JJDUP(J) .EQ.1) then
               ...
               else if(ctab7_j.ne.'missing-'
     .            .and.  (tab_8(8,j).lt.bmiss)
     .            .and. ((tab_8(9,i).ge.bmiss).or.(tab_8(9,j).lt.bmiss))
     .            .and.  (tab_8(10,i).le.3.and.tab_8(10,j).ge.4)) then
     ...
     .                     ' than J'
               ...
               else if( (tab_8(8,j).lt.bmiss)
     .            .and. (tab_8(9,i).ge.bmiss.and.tab_8(9,j).lt.bmiss)
     .            .and. (tab_8(10,i).ge.4.and.tab_8(10,j).le.3)) then
                  ...
                  ! report "J" has a non-missing TOSD and ELEV
                  ! report "I" has a missing TOSD or ELEV.
                  jdup(i) = 1
                  print *, 'I tossed ==> from missing ELEV or TOSD',
     .                     ' in report I'
                  print *,'I,J:',nint(tab_8(10,i)),nint(tab_8(10,j))
               end if
...
C  -----------------------
  ...
  300 FORMAT(/'BUFR_DUPSNO READ IN A TOTAL OF',I8,' REPORTS'/
     .        '  A TOTAL OF ',I7,' REPORTS WERE SKIPPED DUE TO BEING ',
     .        'OVER THE LIMIT OF ',I7//
     .        'BUFR_DUPSNO CHECKED A TOTAL OF',I8,' REPORTS')
      do i = 1,nfiles
...
C  -----------
  ...
  901 CONTINUE
      ...
      PRINT *, '#####BUFR_DUPSNO - UNABLE TO ALLOCATE ARRAYS'
      CALL W3TAGE('BUFR_DUPSNO')
      CALL ERREXIT(99)
