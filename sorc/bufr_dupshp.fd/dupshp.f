...
C  ---------------------------------------------------------------
      ...
      IF(MXTB.EQ.0) THEN
         PRINT *
         PRINT *, '### WARNING: A total of ZERO input ship reports'
         PRINT *
         GO TO 400
      ENDIF
...
C  ---------------------------------------------------
     ...
     .       'flagged as a duplicate - cycle'
         ...
         DUPES =
     . KIDNNT(ABS(TAB_8(1,I)-TAB_8(1,J))*100).LE.NINT(DEXY*100)
     . .AND.
     . KIDNNT(ABS(TAB_8(2,I)-TAB_8(2,J))*100).LE.NINT(DEXY*100)
     . .AND.
     . KIDNNT(ABS(TAB_8(3,I)-TAB_8(3,J))*100.).LE.NINT(DMON*100.)
     . .AND.
     . KIDNNT(ABS(TAB_8(4,I)-TAB_8(4,J))*100.).LE.NINT(DDAY*100.)
     . .AND.
     . KIDNNT(ABS(TAB_8(5,I)-TAB_8(5,J))*100.).LE.NINT(DOUR*100.)
     . .AND.
     . KIDNNT(ABS(TAB_8(6,I)-TAB_8(6,J))*100.).LE.NINT(DMIN*100.)
     . .AND.
     . (crpidI.EQ.crpidJ .OR. (crpidI .EQ. 'SHIP    ' .OR.
     . crpidJ .EQ. 'SHIP    '))
     . .AND.
     . KIDNNT(TAB_8(8,I)).NE.KIDNNT(TAB_8(8,J))
         IF(DUPES) THEN
